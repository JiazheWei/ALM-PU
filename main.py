...
def main():
    ...
    parser.add_argument('--total-steps', default=25 * 2 ** 9, type=int,
                        help='number of total steps to run')##这里改过了  原本是30个epoch
    ...
    parser.add_argument('--warming_steps', default =20 * 2 ** 9, type=int,
                        help='number of epochs in training phase 1')###这里改过了  原本是15
    parser.add_argument('--alpha', default=0.5, type=float,
                        help='mixup parameter on validation set')
    parser.add_argument('--rho', default=0.1, type=float,
                        help='smoothing parameter')
    parser.add_argument('--false_alarm_cutoff', type=float, default=0.05, help='false alarm cutoff')
    ####这里也改过了
    parser.add_argument('--out_constraint_weight', type=float, default=1, help='weight for out-of-distribution penalty in loss function')

    parser.add_argument('--in_constraint_weight', type=float, default=1, help='weight for in-distribution penalty in loss function')
    parser.add_argument('--constraint_tol', type=float,  default=0, help='tolerance for considering constraint violated')
    parser.add_argument('--penalty_mult', type=float, default=1.05, help='multiplicative factor for penalty method')
    ...
    if args.dataset == 'cifar10_1':
        ...
        if args.class_missing:
            ...
            sample, label = train_unlabeled_dataset[0]  # 获取数据集中的一个样本及其标签
            print('#####################################')
            print(sample, label)  # 打印样本和标签以进行检查
        else:
    ...
    #   args.available_label_list = [0,2,3,5,8]
   #    arg.num_classes = 2
   #    train_labled_dataset,train_unlabeled_dataset, test_dataset = get_svhn(args, './dataset')
    #else:
    #        args.available_label_list = [1, 3, 5, 7, 8, 9]
    #        args.num_classes = 6
    #        train_labeled_dataset, train_unlabeled_dataset, test_dataset = get_svhn(args, './dataset')
