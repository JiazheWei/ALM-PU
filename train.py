...
def train_phase1(args, logger, labeled_trainloader, unlabeled_trainloader,
    ...
    for epoch in range(args.start_epoch, args.warming_epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            del logits
            #####上面划分好了有标记的logits和未标记的logits
            
            weights = (torch.sum(logits_x_w.detach(), dim=1) + 1e-8)#这玩意临时加的
            weighth = (torch.sum(logits_x_s.detach(), dim=1) + 1e-8)#这个也是临时加的

            weights = weights.view(-1, 1)
            weighth = weighth.view(-1, 1)

            losses_x_w = F.cross_entropy(logits_x_w, targets_x, reduction='none', label_smoothing=args.rho)
            losses_x_s = F.cross_entropy(logits_x_s, targets_x, reduction='none', label_smoothing=args.rho)
            
            weighted_losses_x_w = losses_x_w * weights
            weighted_losses_x_s = losses_x_s * weighth
            
            Lx = (weighted_losses_x_w.mean() + weighted_losses_x_s.mean()) / 2
            #Lx = (F.cross_entropy(logits_x_w, targets_x, reduction='mean', label_smoothing=args.rho) +
             #原来的这两行     F.cross_entropy(logits_x_s, targets_x, reduction='mean', label_smoothing=args.rho)) / 2
            ...
            record_number = record_number + Ln.item()
            cnt = cnt + 1
            # print(torch.softmax(logits_u.detach() / args.T, dim=-1), targets_u, targets_t)
          
          
            Ln_tempo = Ln - alpha
            if beta * Ln_tempo + lam >= 0:
                special_loss = Ln_tempo * lam + (beta/2 )* (Ln_tempo * Ln_tempo) 
            else :
                special_loss = - (lam * lam )/( 2 * beta )
        ...
        record_number /= cnt###这里相当于更新了那个constraint   说的是要求限制的损失
        in_term_constraint = record_number - args.false_alarm_cutoff
        #in_constraint_weight 就是beta
        lr_lam = 0.98
        # in_term_constraint = in_term_constraint.to(args.device)
        # lam = lam.to(args.device)

        if in_term_constraint * beta + lam >= 0:
            lam += in_term_constraint
        else:
            lam += - lam / beta

        
        if record_number - alpha >= 0 :
